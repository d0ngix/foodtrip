...
class UploadUtil
{
	...
	private function UserPhoto ($requestData) {
		...
		$blnValidUser = $objUserUtil->checkUser($requestData['user_uuid']);
		if ( $blnValidUser !== true ) return "Invalid User";
		...
		try {
			...
			$updataeStatement = $this->db->update( array('photo' => json_encode($data)) )
										->table('users')
										->where('uuid', '=', $requestData['user_uuid']);
		...
		unlink($this->imgPath . $requestData['user_uuid'] . "." . $file->getExtension());
